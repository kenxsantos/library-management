...
if (isset($_POST['new_user'])) {
    ...
    if (count($errors) == 0) {
        ...
        $hashed_password = md5($password);//encrypting the password before saving it to the database
        $query = "INSERT INTO users (username, email, password, userType)
                  VALUES('$hashed_email ', '$email', '$hashed_password', '$userCode')";
...
if (isset($_POST["login_user"])) {
    ...
    if (count($errors) == 0) {
        $password = ($password);
        //encrypting password //turnoff hashing ($password);
...
if (isset($_POST["issueBook"])) {
    ...
    if ($userType_result) {
        ...
        if ($userType_row = mysqli_fetch_assoc($userType_result)) {
            $userType = $userType_row["userType"];
            // Now $userType should have the correct value
        } else {
            // No rows were returned from the query
            // Handle this case, e.g., set a default value for $userType
    ...
    if ($book_pri == "1") {
        ...
        if ($userType == "undergrad") {
           ...
           VALUES('$book_id', '$user_id', '$loaned_on', '$return_by')";
            ...
            if ($loan_results) {
                header('http://localhost:80/LibraryManager-master/user/userProfile/userIssued.php');
            } else {
...
if (isset($_POST["add_waitlist"])) {
    ...
    if (isset($_SESSION["waitlist_book_id"])) {
        // Append the new book_id to the existing list
        $_SESSION["waitlist_book_id"] .= ", " . $book_id;
    } else {
        // Initialize the waitlist_book_id session variable with the current book_id
        $_SESSION["waitlist_book_id"] = $book_id;
    }
    $query = "INSERT INTO waitlist (book_id, user_id) VALUES ($book_id, $user_id)";
    prompt("You have been added to the waiting list");
    // if(isset($_SESSION["waitlist_book_id"]) && !empty($_SESSION["waitlist_book_id"])){
    //     $_SESSION["waitlist_book_id"] = $book_id;
    //     prompt("You have been added to the waiting list");
    // }else{
...
if (isset($_POST["userEditInfo"])) {
    $user_id = mysqli_real_escape_string($db, (isset($_SESSION["user_id"])));//used in the where condition of the update query 
    ...
    if (count($errors) === 0) {
        ...
        mysqli_query($db, $query);
        header("http://localhost:80/LibraryManager-master/php/user/userProfile/userProfileInfo.php");
    }
